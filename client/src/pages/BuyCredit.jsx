...
const BuyCredit = () => {
  ...
  const paymentRazorpay = async (planId) => {
    try {
      if (!user) {
        setShowLogin(true);
        return; // Prevent execution if user is not logged in
      }
      ...
      if (data.success && data.message) { // ✅ Check if message contains order data
        initPay(data.message); // ✅ Pass data.message instead of data.order
      } else {
